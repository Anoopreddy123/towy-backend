...
class GeoService {
    ...
    async createServiceRequest(data) {
        ...
        const result = await this.db.query(`INSERT INTO service_requests (
    ...
    async ensureUserExists(userId) {
        try {
            ...
            const checkResult = await this.db.query('SELECT id FROM users WHERE id = $1', [userId]);
            if (checkResult.rows.length === 0) {
                // Create a minimal user record
                await this.db.query('INSERT INTO users (id, name, email, password, role, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, NOW(), NOW())', [userId, 'Unknown User', `user-${userId}@temp.com`, 'temp-password', 'customer']);
    ...
    async getUserRequests(userId) {
        ...
        try {
            const result = await this.db.query('SELECT * FROM service_requests WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
    ...
    async getServiceRequest(requestId) {
        const result = await this.db.query('SELECT * FROM service_requests WHERE id = $1', [requestId]);
    ...
    async updateServiceStatus(requestId, status) {
        ...
        const result = await this.db.query('UPDATE service_requests SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *', [status, requestId]);
        if (result.rows.length === 0) {
            return null;
        }
        const row = result.rows[0];
    ...
    async loginProvider(email, password) {
        try {
            ...
            const token = (0, jsonwebtoken_1.sign)({
                ...
                role: 'provider'
            }, process.env.JWT_SECRET || 'your_secret_key', { expiresIn: '24h' });
    ...
    async getNearbyRequestsByProvider(providerId, miles = 50) {
        // 1 mile = 1609.34 meters
        const meters = miles * 1609.34;
        // Ensure provider exists and get location
        const providerResult = await this.db.query(`SELECT location FROM providers WHERE id = $1`, [providerId]);
        if (providerResult.rows.length === 0) {
            throw new Error('Provider not found');
        }
        // Query pending service requests within radius of provider.location
        const requestsResult = await this.db.query(`SELECT 
                sr.*,
                ST_Distance(
                    p.location,
                    ST_MakePoint(
             ...
             ORDER BY distance_meters ASC`, [providerId, meters]);
